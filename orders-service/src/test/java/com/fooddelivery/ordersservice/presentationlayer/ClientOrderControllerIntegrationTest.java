...
class ClientOrderControllerIntegrationTest {
    ...
    public void testGetAllOrdersByClientId() throws Exception {
        ...
        OrderResponseModel responseModel = new OrderResponseModel(*//*("test-order-id", clientId, "test-restaurant-id",
                "test-menu-id","delivery-driver-id","testFirstName","testLastName","testUserName","testEmail",items,"testRestaurantName","Appetizers", OrderStatus.MAKING_ORDER,20.0,"20 minutes" , LocalDate.of(2020,12,12)*//*);

        List<OrderResponseModel> mockResponse = Collections.singletonList(responseModel);

        // Mock the external service response
        mockRestServiceServer.expect(ExpectedCount.once(), requestTo(new URI("http://orders-service:7004/api/v1/clients/" + clientId + "/orders")))
        ...
        webTestClient.get()
                .uri("/api/v1/clients/" + clientId + "/orders")
...
/*    @Test
    public void deleteOrderTest() {
        ...
        // Create order first if your service allows that
        // restTemplate.postForEntity("/api/v1/orders", request, Order.class);

        // Then delete
        ResponseEntity<Void> responseEntity = restTemplate.delete("/api/v1/orders/" + orderId);
